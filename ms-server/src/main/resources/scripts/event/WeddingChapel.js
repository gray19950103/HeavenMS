...
function spawnCakeBoss(eim) {
        ...
        var mobObj = Packages.asia.wmj.ms.server.life.MapleLifeFactory.getMonster(9400606);
...
function sendWeddingAction(eim, type) {
        ...
        if(chr.getGender() == 0) {
                chr.getMap().broadcastMessage(Packages.asia.wmj.ms.tools.packets.Wedding.OnWeddingProgress(type == 2, eim.getIntProperty("groomId"), eim.getIntProperty("brideId"), type + 1));
        } else {
                chr.getMap().broadcastMessage(Packages.asia.wmj.ms.tools.packets.Wedding.OnWeddingProgress(type == 2, eim.getIntProperty("brideId"), eim.getIntProperty("groomId"), type + 1));
        }
}
...
function showStartMsg(eim) {
        eim.getMapInstance(entryMap + 10).broadcastMessage(Packages.asia.wmj.ms.tools.packets.Wedding.OnWeddingProgress(false, 0, 0, 0));
        eim.schedule("hidePriestMsg", forceHideMsgTime * 1000);
}

function showBlessMsg(eim) {
        eim.getMapInstance(entryMap + 10).broadcastMessage(Packages.asia.wmj.ms.tools.packets.Wedding.OnWeddingProgress(false, 0, 0, 1));
