...
function start() {
    ...
    if (jobType >= minJobType && jobType <= maxJobType && cm.canSpawnPlayerNpc(Packages.asia.wmj.ms.constants.game.GameConstants.getHallOfFameMapid(cm.getJob()))) {
        spawnPnpc = true;
    ...
    if (status == -1){
        ...
        if(spawnPnpc) {
            if(mode > 0) {
                ...
                if(Packages.asia.wmj.ms.server.life.MaplePlayerNPC.spawnPlayerNPC(Packages.asia.wmj.ms.constants.game.GameConstants.getHallOfFameMapid(cm.getJob()), cm.getPlayer())) {
