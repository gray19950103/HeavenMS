    ...
    if (mode < 0)
        ...
        if (status == 0 && mode == 1) {
			...
			if(choice == 0) {
				...
				cm.getPlayer().announce(Packages.asia.wmj.ms.tools.MaplePacketCreator.earnTitleMessage("You have earned " + nxAmount + " NX"));
				cm.logLeaf(nxAmount + " NX");
				cm.dispose();
			} else if(choice == 2) {
				if(!cm.getPlayer().getInventory(Packages.asia.wmj.ms.client.inventory.MapleInventoryType.SETUP).isFull(chairAmount)) {
				...
				if(!cm.getPlayer().getInventory(Packages.asia.wmj.ms.client.inventory.MapleInventoryType.EQUIP).isFull(weaponAmount)) {
				...
				if(!cm.getPlayer().getInventory(Packages.asia.wmj.ms.client.inventory.MapleInventoryType.USE).isFull(2)) {
				...
				if(!cm.haveItem(5030000, 1)) {
					if(!cm.getPlayer().getInventory(Packages.asia.wmj.ms.client.inventory.MapleInventoryType.CASH).isFull(1)){
